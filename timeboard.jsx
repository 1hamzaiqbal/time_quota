...
import { Play, Pause, RotateCcw, Plus, Trash2, GripHorizontal, Clock, AlertCircle, FileText, X, History, Palette, ChevronRight } from 'lucide-react';

// --- Color Palette ---
const COLORS = [
  { bg: 'bg-rose-100', border: 'border-rose-200', text: 'text-rose-800', bar: 'bg-rose-500', darkBar: 'bg-rose-600', modalBg: 'bg-rose-50', dot: 'bg-rose-500' },
  { bg: 'bg-orange-100', border: 'border-orange-200', text: 'text-orange-800', bar: 'bg-orange-500', darkBar: 'bg-orange-600', modalBg: 'bg-orange-50', dot: 'bg-orange-500' },
  { bg: 'bg-amber-100', border: 'border-amber-200', text: 'text-amber-800', bar: 'bg-amber-500', darkBar: 'bg-amber-600', modalBg: 'bg-amber-50', dot: 'bg-amber-500' },
  { bg: 'bg-emerald-100', border: 'border-emerald-200', text: 'text-emerald-800', bar: 'bg-emerald-500', darkBar: 'bg-emerald-600', modalBg: 'bg-emerald-50', dot: 'bg-emerald-500' },
  { bg: 'bg-teal-100', border: 'border-teal-200', text: 'text-teal-800', bar: 'bg-teal-500', darkBar: 'bg-teal-600', modalBg: 'bg-teal-50', dot: 'bg-teal-500' },
  { bg: 'bg-cyan-100', border: 'border-cyan-200', text: 'text-cyan-800', bar: 'bg-cyan-500', darkBar: 'bg-cyan-600', modalBg: 'bg-cyan-50', dot: 'bg-cyan-500' },
  { bg: 'bg-blue-100', border: 'border-blue-200', text: 'text-blue-800', bar: 'bg-blue-500', darkBar: 'bg-blue-600', modalBg: 'bg-blue-50', dot: 'bg-blue-500' },
  { bg: 'bg-indigo-100', border: 'border-indigo-200', text: 'text-indigo-800', bar: 'bg-indigo-500', darkBar: 'bg-indigo-600', modalBg: 'bg-indigo-50', dot: 'bg-indigo-500' },
  { bg: 'bg-violet-100', border: 'border-violet-200', text: 'text-violet-800', bar: 'bg-violet-500', darkBar: 'bg-violet-600', modalBg: 'bg-violet-50', dot: 'bg-violet-500' },
  { bg: 'bg-fuchsia-100', border: 'border-fuchsia-200', text: 'text-fuchsia-800', bar: 'bg-fuchsia-500', darkBar: 'bg-fuchsia-600', modalBg: 'bg-fuchsia-50', dot: 'bg-fuchsia-500' },
];
...
export default function App() {
  ...
  const [tasks, setTasks] = useState(() => {
    ...
    if (saved) {
      ...
      return parsed.map(t => ({ notes: '', ...t }));
    }
    return [
      { id: '1', title: 'Deep Work', quotaMinutes: 60, elapsed: 0, isRunning: false, colorIndex: 7, notes: '- Finish the Q3 report\n- Email the design team' },
      { id: '2', title: 'Email Triaging', quotaMinutes: 15, elapsed: 0, isRunning: false, colorIndex: 4, notes: '' },
    ];
  });
  ...
  const addManualTime = (id, minsToAdd) => {
    setTasks(tasks.map(t => 
        t.id === id ? { ...t, elapsed: Math.max(0, t.elapsed + (minsToAdd * 60)) } : t
    ));
  };
    ...
    <div className="min-h-screen bg-slate-900 text-slate-100 p-4 sm:p-8 font-sans selection:bg-slate-700">
      ...
      <div className="max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 pb-20">
        {tasks.map((task, index) => {
          ...
          const progressPercent = Math.min(100, (task.elapsed / quotaSeconds) * 100);
          ...
          return (
            ...
            >
              {/* Drag Handle */}
              <div className="absolute top-4 right-4 opacity-0 group-hover:opacity-100 transition-opacity text-slate-400/50">
              ...
              <div className="space-y-4 z-10">
                <input 
                  ...
                  className={`w-full bg-transparent text-xl font-bold ${colors.text} placeholder-slate-400/50 outline-none border-none focus:ring-0 p-0 truncate`}
              ...
              <div className="space-y-4 z-10">
                ...
                <div className="h-3 w-full bg-white/40 rounded-full overflow-hidden relative">
                  ...
                  />
                  {isOvertime && (
                    <div className="absolute inset-0 bg-[linear-gradient(45deg,transparent_25%,rgba(255,255,255,0.3)_50%,transparent_75%)] bg-[length:20px_20px] animate-stripes" />
                  )}
                </div>
                ...
                <div className="flex items-center justify-between pt-2">
                  <div className="flex gap-2">
                    ...
                    <button 
                      ...
                      className={`p-3 rounded-xl transition-colors hover:bg-white/50 ${colors.text}`}
                      title="Reset"
                    >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/80 backdrop-blur-sm animate-in fade-in duration-200">
          ...
          >
              ...
              <div className="flex-1 mr-4">
                ...
                <div className="flex items-center gap-4 mt-2 text-sm font-medium opacity-75">
                   ...
                   <span className={`${activeColors.text} font-mono`}>
                      Current: {formatTime(activeTask.elapsed)}
                   </span>
                </div>
              </div>

              {/* Controls: Color Picker + Close */}
              <div className="flex items-center gap-3">
                
                {/* Color Picker (Moved & Fixed) */}
            ...
            <div className="flex-1 overflow-hidden flex flex-col md:flex-row">
              ...
              {/* Sidebar Controls (Desktop) / Bottom (Mobile) */}
              <div className={`p-6 md:w-72 flex flex-col gap-6 border-t md:border-t-0 md:border-l ${activeColors.border} bg-white/10 overflow-y-auto`}>
                ...
                {/* Timer Display Large */}
                <div className="text-center p-6 bg-white/30 rounded-2xl shadow-sm">
                  <div className={`text-4xl font-mono font-bold tracking-tight mb-2 ${activeTask.elapsed > activeTask.quotaMinutes * 60 ? 'text-red-600' : activeColors.text}`}>
                  ...
                  <div className="flex justify-center gap-2">
                    ...
                    <button 
                      ...
                      className={`p-2 rounded-lg bg-white/50 hover:bg-white/80 transition-colors ${activeColors.text}`}
                    >
                      <RotateCcw className="w-5 h-5" />
                    </button>
                  </div>
